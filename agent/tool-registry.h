...
public:
  ...
  to_chat_tools_filtered(const std::set<std::string> &allowed_tools) const;

  // Execute a tool by name
  tool_result execute(const std::string &name, const json &args,
                      const tool_context &ctx) const;

  tool_result
  execute_filtered(const std::string &name, const json &args,
...
// 注册工具宏：在全局静态区实例化 tool_registrar，
// 借助其构造函数自动将工具注册到 tool_registry
