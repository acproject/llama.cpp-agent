...
bool permission_manager_async::matches_pattern(
...
bool permission_manager_async::is_path_in_project(
...
permission_state
permission_manager_async::check_permission(const permission_request &request) {
...
std::string permission_manager_async::request_permission(
...
bool permission_manager_async::respond(const std::string &request_id,
                                       bool allowed, permission_scope scope) {
  std::lock_guard<std::mutex> lock(mutex_);
  ...
  if (scope == permission_scope::SESSION) {
    ...
    session_overrides_[key] = allowed ? permission_state::ALLOW_SESSION
                                      : permission_state::DENY_SESSION;
  }
...
std::optional<permission_response_async>
permission_manager_async::wait_for_response(const std::string &request_id,
                                            int timeout_ms) {
...
void permission_manager_async::record_tool_call(const std::string &tool,
...
bool permission_manager_async::is_doom_loop(
    const std::string &tool, const std::string &args_hash) const {
  std::lock_guard<std::mutex> lock(mutex_);
  if (recent_calls_.empty())
    return false;
