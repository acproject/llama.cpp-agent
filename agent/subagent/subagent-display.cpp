...
void subagent_display::print_header(int depth, const std::string &icon,
  ...
  if (buffer) {
    buffer->write(DISPLAY_TYPE_RESET, "\n%s%s%s%s ", prefix.c_str(),
                  TREE_CORNER_TOP, TREE_HORIZONTAL, TREE_HORIZONTAL);
    ...
    if (!description.empty()) {
      buffer->write(DISPLAY_TYPE_RESET, " %s%s", description.c_str(),
                    TREE_VERTICAL);
      buffer->write(DISPLAY_TYPE_REASONING, "%s\b", description.c_str());
    }
  } else {
    // Direct mode: use output_guard for atomic output
    output_guard guard;

    guard.write("\n%s%s%s%s ", prefix.c_str(), TREE_CORNER_TOP, TREE_HORIZONTAL,
                TREE_HORIZONTAL);
...
void subagent_display::print_tool_call(int depth, const std::string &tool_name,
  ...
  if (buffer) {
    ...
    if (!args_summary.empty()) {
      buffer->write(DISPLAY_TYPE_INFO, "%s", args_summary.c_str());
    }
    buffer->write(DISPLAY_TYPE_RESET, " ");
    if (elapsed_ms < 1000) {
...
// Helper to format token count (e.g., 1234 -> "1.2k", 123456 -> "12.3k")
...
subagent_display::scope::scope(subagent_display &display,
                               const std::string &name, subagent_type type,
                               const std::string &description)
    : display_(display), buffer_(nullptr) {
...
subagent_display::scope::scope(subagent_display &display,
                               ...
                               const std::string &description,
                               subagent_output_buffer *buffer)
    : display_(display), buffer_(buffer) {
        std::lock_guard<std::mutex> lock(display_.mtx_);
        depth_ = display_.depth_.fetch_add(1);
