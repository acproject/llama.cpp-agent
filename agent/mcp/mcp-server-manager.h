...
public:
  ...
  // Load configuation from JSON file
  ...
  // Returns paris of (qulified_name, tool)
...
private:
  ...
  static std::string qualify_name(const std::string &server,
                                  const std::string &tool);
  static bool parse_qualified_name(const std::string &qualified,
                                   std::string &server, std::string &tool);
...
// Check ./mcp.json, then ~/.llama-agent/mcp.json
