...
#include <cstdlib>    // 用于调用 std::system 启动 mcp-server 进程
#include <memory>
...
bool mcp_server_manager::load_config(const std::string &config_path) {
  ...
  for (auto &[name, server_json] : config["servers"].items()) {
    ...
    cfg.timeout_ms = server_json.value("timeout_ms", 60000);
...
std::vector<std::pair<std::string, mcp_tool>> mcp_server_manager::list_all_tools() {
  ...
  for (auto& [server_name, client] : clients_) {
    ...
    for (auto &tool : tools) {
      ...
      result.push_back({qualified, tool});
    }
  }
  return result;
}
...
std::string mcp_server_manager::qualify_name(const std::string &server,
                                             const std::string &tool) {
  // Replace any __ in server/tool names with _ to avaid confusion
...
bool mcp_server_manager::parse_qualified_name(const std::string &qualified_name,
                                             std::string &server,
                                             std::string &tool) {
  // Format: mcp__<server>__<tool>
  const std::string prefix = "mcp__";
  if (qualified_name.substr(0, prefix.size()) != prefix) {
    return false;
  }

  std::string rest = qualified_name.substr(prefix.size());
...
std::string mcp_server_manager::expand_env_vars(const std::string &value) {
  ...
  while (std::regex_search(result, match, env_regex)) {
    ...
    result = result.substr(0, match.position()) +
             replacement + result.substr(match.position() + match.length());
  }
  return result;
}
