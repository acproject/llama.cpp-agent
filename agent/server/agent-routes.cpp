...
server_http_res_ptr agent_routes::make_error(int status,
  ...
  sse_stream_res() {
    ...
    next = [this](std::string& output) -> bool {
      ...
      cv.wait_for(lock, std::chrono::milliseconds(100),
                  [this] { return !chunks.empty() || done.load(); });
...
struct sse_shared_wrapper: server_http_res {
  ...
  explicit sse_shared_wrapper(std::shared_ptr<sse_stream_res> s)
      : sse(std::move(s)) {
    content_type = sse->content_type;
    headers = sse->headers;
    next = [this](std::string &output) -> bool {
      return this->sse->next(output);
    };
  }
};
...
agent_routes::agent_routes(agent_session_manager &session_mgr)
  ...
  post_session = [this](const server_http_req &req) -> server_http_res_ptr {
    ...
    if (!req.body.empty()) {
      try {
        ...
        if (body.contains("skill_paths") && body["skills_paths"].is_array()) {
  ...
  get_session = [this](const server_http_req &req) -> server_http_res_ptr {
    ...
    if (session_id.empty()) {
      return make_error(404, "Missing session ID");
    }
  ...
  // Supports both text-only and multimodal content (OpenAI format)
  post_chat = [this](const server_http_req &req) -> server_http_res_ptr {
    ...
    // Parse message content from body - supports text or multimodal
    json body;
    json user_message;
  ...
  // GET /v1/agent/session/:id/message - Get conversation history
  ...
  // GET /v1/agent/session/:id/permission - Get pending permissions
  ...
  post_permission = [this](const server_http_req &req) -> server_http_res_ptr {
    ...
    try {
      ...
      if (!body.contains("allow")) {
        return make_error(400, "Miss 'allow' field");
      }
    ...
    // (In a real implementation, you'd want a permission_id -> session_id
    // mapping)
  ...
  get_tools = [this](const server_http_req &req) -> server_http_res_ptr {
    auto tools = tool_registry::instance().to_chat_tools();
    json response = json::array();
    for (const auto &tool : tools) {
      response.push_back({{"name", tool.name},
...
// Register all agent rountes with HTTP context
void register_agent_routes(server_http_context &ctx, agent_routes &routes) {
  ...
  ctx.post("/v1/agent/session/:id/chat", routes.delete_session);
