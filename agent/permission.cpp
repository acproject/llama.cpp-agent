...
permission_manager::permission_manager() {
  ...
  // Dangerous bash patterns (always aks with warning)
  dangerous_patterns_ = {// Destructive commands
                         ...
                         // System damage
                         "> /dev/", "dd if=", "mkfs.", ":(){:|:&};:",
                         // Package managers (cand modify system)
                         "pip install", "pip3 install", "npm i -g",
                         "npm install -g", "brew install", "apt install",
                         "apt-get install", "yum install",
                         // Git destructive
                         "git push -f", "git push --force", "git rest --hard",
                         // Process control
                         "kill -9", "killall", "pkill"};

  // Safe bash patterns (auto-allow)
  safe_patterns_ = {"ls",      "pwd",      "cat ",       "head ", "tail ",
                    "grep ",   "find ",    "wc ",        "diff ", "git status",
                    "git log", "git diff", "git branch", "echo ", "which ",
                    "type ",   "file "
...
bool permission_manager::matches_pattern(
    const std::string &cmd, const std::vector<std::string> &patterns) const {
...
bool permission_manager::is_path_in_project(const std::string &path) const {
  ...
  try {
    ...
    // excat match is always in project
    if (abs_path == project_root_)
      return true;
    // Check that path is within project_root directory (not just a prefix
    // match)
...
permission_state permission_manager::check_permission(const permission_request &request) {
  ...
  // For MCP tool, use tool name only (argument vary per call)
  // For other tools, include details for more granular contorl
  std::string key = request.tool_name.rfind("mcp__", 0) == 0
                        ? request.tool_name
                        : request.tool_name + ":" + request.details;
  auto it = session_override_.find(key);
  if (it != session_override_.end()) {
    return it->second;
  }
...
permission_response permission_manager::prompt_user(const permission_request &request) {
  ...
  for (size_t i = request.tool_name.length() + 16; i < 60; i++)
    console::log("-");
  console::log("\n");
  ...
  if (ch == 'a' || ch == 'A') {
    ...
    // For MCP tools, use tool name only (argument vary per call)
    std::string key = request.tool_name.rfind("mcp__", 0) == 0
                          ? request.tool_name
                          : request.tool_name + ":" + request.details;
    session_override_[key] = permission_state::ALLOW_SESSION;
  ...
  if (ch == 'd' || ch == 'D') { // 'd' for deny always(since 'N' is deny once)
    ...
    std::string key = request.tool_name.rfind("mcp__", 0) == 0
                          ? request.tool_name
                          : request.tool_name + ":" + request.details;
    session_override_[key] = permission_state::DENY_SESSION;
...
void permission_manager::record_tool_call(const std::string &tool,
                                          const std::string &args_hash) {
...
bool permission_manager::is_doom_loop(const std::string &tool,
                                      const std::string &args_hash) const {
  if (recent_calls_.empty())
    return false;
...
bool permission_manager::is_sensitive_file(const std::string &path) {
  ...
  if (filename_lower.find("aws") != std::string::npos &&
      (filename_lower.find("credentials") != std::string::npos ||
