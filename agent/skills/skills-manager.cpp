...
parse_yaml_line(const std::string &line) {
...
bool skill_manager::validate_name(const std::string &name) {
  ...
  // No XML tags allowed (spec requirment)
...
skill_manager::parse_skill(const std::string &skill_dir) {
  ...
  fs::path scripts_dir = fs::path(scripts_dir) / "scripts";
    ...
    try {
      for (const auto &entry : fs::directory_iterator(search_path)) {
        ...
        if (skill) {
          ...
          for (const auto &existsing : skills_) {
            if (existsing.name == skill->name) {
              duplicate = true;
              break;
            }
          }
...
std::string skill_manager::generate_prompt_section() const {
  ...
  for (const auto &skill : skills_) {
    ...
    xml << " <description>" << escape_xml(skill.description)
        << "</description>\n";
    xml << " <location>" << escape_xml(skill.skill_dir) << "</location>\n";
