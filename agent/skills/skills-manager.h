...
struct skill_metadata {
  std::string name;             // Required: 1-64 char, lowercase+numbers+hyphens
  ...
  std::vector<std::string> scripts;        // Relative paths like "script/analyze.py"
  std::vector<std::string> allowed_tools;  // Experimental tools like skill can use
};
