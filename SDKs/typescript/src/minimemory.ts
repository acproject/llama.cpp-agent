...
function encodeBulk(s: string): Buffer {
...
function parseIntLine(buf: Buffer, start: number): { n: number; next: number } | null {
...
function parseStringLine(buf: Buffer, start: number): { s: string; next: number } | null {
  const end = indexOfCrlf(buf, start);
  if (end < 0) return null;
  const s = buf.subarray(start, end).toString("utf8");
  return { s, next: end + 2 };
...
function parseResp(buf: Buffer, start = 0): { value: RespValue; next: number } | null {
  ...
  if (lead === 42) {
    ...
    if (count === -1) return { value: { type: "array", value: null }, next: countLine.next };
...
export function respToJson(v: RespValue): any {
  if (v.type === "simple" || v.type === "error") return v.value;
  if (v.type === "int") return v.value;
  if (v.type === "bulk") return v.value;
  if (v.type === "array") return v.value ? v.value.map(respToJson) : null;
  return null;
}
...
export class MiniMemoryClient {
  ...
  close(): void {
    ...
    while (this.pending.length > 0) {
      ...
      if (p) p.reject(new Error("socket closed"));
    }
  }
  ...
  async command(args: string[]): Promise<RespValue> {
    ...
    const p = new Promise<RespValue>((resolve, reject) => this.pending.push({ resolve, reject }));
    sock.write(payload);
    return p;
  }
}
